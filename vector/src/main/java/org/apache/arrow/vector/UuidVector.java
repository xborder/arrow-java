...
/**
 * Production UuidVector wrapping a FixedSizeBinaryVector with width 16, logically typed as
 * UuidType.
 */
public class UuidVector extends ExtensionTypeVector<FixedSizeBinaryVector>
  ...
  public UUID getObject(int index) {
    final byte[] bytes = getUnderlyingVector().getObject(index);
    if (bytes == null) {
      return null;
    }
  ...
  protected FieldReader getReaderImpl() {
    // Use underlying reader; specific reader impl is optional for this production vector
    return getUnderlyingVector().getReader();
  }
